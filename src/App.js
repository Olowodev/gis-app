...
function App() {
  ...
  const [tilesUrl, setTilesUrl] = useState('http://godzilla.bk.tudelft.nl/3dtiles/ZuidHolland/lod13/tileset1.json')
  ...
  const initScene = () => {
    let renderer, scene, offsetParent, camera, dummyCamera, controls, material, highlightMaterial, raycaster, mouse, box, tiles, fog, pane, selectedObject, sceneTransform, rayIntersect = null
    let needsRenderer, lruCacheSize, dirZ, errorTarget = 0
    ...
    function renderScene () {
      ...
      if ( needsRenderer > 0 ) {
        ...
        lruCacheSize = tiles.lruCache.itemSet.sizeAttenuation
